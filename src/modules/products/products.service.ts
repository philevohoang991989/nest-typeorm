...
export class ProductsService {
  ...
  async create(dto: CreateProductDto): Promise<any> {
    ...
    // 3. Xử lý attributes
    const valuesToSave: ProductAttributeValue[] = [];
    ...
    for (const attrInput of dto.attributes) {
      ...
      if (attrInput.attributeId) {
        ...
        if (!attribute) {
          throw new BadRequestException(`Attribute ID ${attrInput.attributeId} is invalid for this product type`);
        }
      }
      ...
      // CASE 3: Không có attributeId và không có attribute.name
        ...
        throw new BadRequestException('Missing attributeId or attribute.name in one of the attributes');
      }
